...
| `linkTextSpacing` | `linkTextSpacing` | Collapse internal whitespace and trim leading/trailing whitespace inside link/reference-link text; `all` (default) also turns line breaks (soft and hard) into a space, `spaces` leaves any run containing a line break as written, `preserve` leaves link text alone |
...
| `blockquoteMarkerSpacing` | `normalizeBlockquoteMarkerSpacing` | Use one space between blockquote `>` marker(s) and quoted content; leaves indentation before/between markers, protected content, and lists inside blockquotes untouched |
