...
| `linkTextSpacing`        | `normalizeLinkTextSpacing`                               | Collapse internal whitespace and trim leading/trailing whitespace inside link/reference-link text (soft newlines become a space); only `text` children are touched |
